...
export function MatchWorkflow() {
  ...
  return <div className="space-y-12"><form onSubmit={submit} className="glass rounded-3xl p-6 sm:p-8"><div className="grid gap-5 md:grid-cols-2"><Field label="Article title"><input required className="field" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="How red foxes survive winter" /></Field><Field label="Expected subject"><input className="field" value={form.expected_subject ?? ""} onChange={(event) => setForm({ ...form, expected_subject: event.target.value })} placeholder="red fox" /></Field><Field wide label="Article body"><textarea required rows={6} className="field resize-y" value={form.body} onChange={(event) => setForm({ ...form, body: event.target.value })} placeholder="Write or paste the article text…" /></Field><Field label="Expected category"><input className="field" value={form.expected_category ?? ""} onChange={(event) => setForm({ ...form, expected_category: event.target.value })} placeholder="animal" /></Field><Field label="Required tags"><input className="field" value={tagText} onChange={(event) => setTagText(event.target.value)} placeholder="snow, winter" /><span className="muted mt-1 block text-xs">Comma-separated</span></Field></div><button disabled={stage !== null} className="btn-primary mt-6" type="submit">{stage ? <><span className="size-4 animate-spin rounded-full border-2 border-black/20 border-t-black" />{stage}…</> : "Create & match article"}</button></form>
